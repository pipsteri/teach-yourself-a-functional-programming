object Recursion {
    ...
    /** computes factorial of a number
      ...
      * 3! = 3 * 2 * 1
      */
    def fact(num:Int) =  if (num==0) 1 else num * fact(num-1)
    ...
    /** computes fibonacci value for a given number
      ...
      */
     def fib(num:Int):Int = num match { case 0 => 0  case 1 => 1 case _ => fib(num-1) + fib(num-2) }

    /** returns true if 'list' contains 'needle'
      *
      * Tip: for maximum learning, implement list traversal via recursion
      */
       // almost works: gives true instead of false and vice versa
    def contains[A](list: List[A], needle: A):Boolean = list.contains("needle") == true
